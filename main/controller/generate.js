...
export const preparationData = (properties) => {
  ...
  const promise = new CPromise((resolve, reject, { onCancel }) => {
    ...
    reader.on("end", () => {
      ...
      // add month to properties, month start from zero in previous data
      properties.month += 1;
      // faktur preparation
      const fakturs = [];
      properties.faktur.forEach((faktur) => {
        const fakturMin = parseInt(faktur[0].replace(new RegExp("-", "g"), ""));
        const fakturMax = parseInt(faktur[1].replace(new RegExp("-", "g"), ""));
        for (let index = fakturMin; index <= fakturMax; index++) {
          if (fakturs.indexOf(index) < 0) {
            fakturs.push(Transform.number.zpad(index, 13));
          }
        }
      });
      properties.faktur = [...fakturs];
...
export const processDataBook = async (
  ...
  if (!isRunning) return;
  try {
    classController = await import(`../controller/companies/${company.Class}`);
  } catch (error) {
    // TODO: create log here
    throw error;
  }
